	...
	<view class="content">
		<form class="app-update-pv">
			<view
                ...
                <textarea
                        :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"#f06f4f","borderRadius":"0px","color":"#f06f4f","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}'
                        v-model='ruleForm.shangpinCommentbackText' placeholder="留言内容"></textarea>
            </view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				let res = await this.$api.info(`shangpinCommentback`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
			if ((!this.ruleForm.shangpinCommentbackText)) {
				this.$utils.msg(`留言内容不能为空`);
				return
			}
				if (this.ruleForm.id) {
					await this.$api.update(`shangpinCommentback`, this.ruleForm);
				} else {
					await this.$api.save(`shangpinCommentback`, this.ruleForm);
				}
