<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.kaoyanName" type="text" placeholder="考研信息标题"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in kaoyanTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.kaoyanName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"80rpx","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.kaoyanPhoto?product.kaoyanPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.kaoyanNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#fff","textAlign":"center"}'>
            <text class="">￥{{product.kaoyanNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('kaoyan','修改') && product.yonghuId == user.id" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('kaoyan','删除') && product.yonghuId == user.id" class="cuIcon-delete"
            ...
            v-if="isAuth('kaoyan','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 考研信息类型 字段 字典表查询方法
		  	let kaoyanTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'kaoyan_types'
		  	});
		  	this.kaoyanTypesList = kaoyanTypes.data.list;
		...
		methods: {
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.kaoyanTypes = this.categoryName
                            }

                let res = await this.$api.list(`kaoyan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('kaoyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.kaoyanName) {
                    searchForm['kaoyanName'] = this.searchForm.kaoyanName
                }
				if (this.searchForm.kaoyanName) {
					searchForm['kaoyanName'] = this.searchForm.kaoyanName
				}

                let res = await this.$api.list(`kaoyan`, searchForm);
				// 如果是第一页数据置空
